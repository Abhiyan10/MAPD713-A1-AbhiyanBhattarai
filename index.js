let SERVER_FULLNAME = "product-api";
let PORT = 3000;
let HOST = "127.0.0.1";
...
let server = restify.createServer({ name: SERVER_FULLNAME });

// Initialize req counter
...
//  logging requests and responses and updating request counters
...
server.get("/products/:_id", function (req, res, next) {
  ...
  productsSave.findOne({ _id: req.params._id }, function (error, product) {
    // If errors occurs, pass to next  correct format
    if (error) return next(new Error(JSON.stringify(error.errors)));

    if (product) {
      // Send the product if there is no issues
      res.send(product);
    } else {
      // Send 404 header if the product don't exist
      res.send(404);
    }
  });
});
...
server.post("/products", function (req, res, next) {
  ...
  productsSave.create(newProduct, function (error, product) {
    // If there occurs errors, pass to next correct format
    if (error) return next(new Error(JSON.stringify(error.errors)));

    // Send the product if occcurs no issues
